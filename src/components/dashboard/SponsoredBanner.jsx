...
export default function SponsoredBanner({ deals }) {
  ...
  const Inner = (
    <div className="mx-5 mt-10 rounded-2xl overflow-hidden border border-border/30 bg-card flex items-center gap-4 px-5 py-4 shadow-luxe-sm">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[10px] font-body tracking-luxe-xs uppercase text-muted-foreground/60">Partner</p>
      ...
      <ArrowRight className="w-4 h-4 text-ocean flex-shrink-0" strokeWidth={1.5} />
      {sponsored.length > 1 && (
        <div className="flex gap-1.5 flex-shrink-0 ml-1">
          {sponsored.map((_, i) => (
            <button
              ...
              className={`w-1.5 h-1.5 rounded-full transition-colors ${i === bannerIdx ? 'bg-ocean' : 'bg-border'}`}
